...
// Presets rápidos de color primario (deben incluir el valor por defecto).
const THEME_PRESETS = [
  { name: 'Azul', color: '#2f68d8' },
  { name: 'Esmeralda', color: '#2f8f5b' },
  { name: 'Violeta', color: '#6d5ae0' },
  { name: 'Naranja', color: '#e0662f' },
  { name: 'Rojo', color: '#c23b2f' },
  { name: 'Grafito', color: '#3a3a42' },
];
...
export function SettingsPage() {
  ...
  const { data } = useQuery({ queryKey: ['business', 'edit'], queryFn: () => api.get<BusinessConfig>('/business/me') });
  ...
  useEffect(() => {
    ...
    root.setProperty('--color-primary', form.primary);
    root.setProperty('--color-secondary', form.secondary);
  ...
  useEffect(() => {
    return () => {
      const root = document.documentElement.style;
      root.setProperty('--color-primary', savedTheme?.primary ?? DESIGN_DEFAULTS.primary);
      root.setProperty('--color-secondary', savedTheme?.secondary ?? DESIGN_DEFAULTS.secondary);
    ...
    <div className="mx-auto flex max-w-2xl flex-col gap-4 p-4">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-5">
          ...
          {/* Presets rápidos de color primario. */}
          <div className="flex flex-wrap gap-2">
            {THEME_PRESETS.map((t) => {
              const active = form.primary.toLowerCase() === t.color.toLowerCase();
              return (
                <button
                  key={t.color}
                  type="button"
                  onClick={() => setForm((f) => ({ ...f, primary: t.color }))}
                  className={`inline-flex items-center gap-2 rounded-theme border px-3 py-2 text-sm font-semibold ${
                    active ? 'border-primary bg-primary/10 text-fg' : 'border-border bg-surface text-muted hover:bg-muted/10'
                  }`}
                >
                  <span className="inline-block h-4 w-4 rounded-full" style={{ background: t.color }} />
                  {t.name}
                  {t.color === DESIGN_DEFAULTS.primary && <span className="text-[10px] text-muted">· def.</span>}
                </button>
              );
            })}
          </div>
          ...
          <div className="flex flex-wrap gap-6">
            <ColorField label="Primario" value={form.primary} onChange={(v) => setForm({ ...form, primary: v })} />
            <ColorField label="Secundario" value={form.secondary} onChange={(v) => setForm({ ...form, secondary: v })} />
          </div>

          {/* Radio de bordes. */}
          <div className="flex flex-col gap-2">
            <label className="text-sm text-muted">Radio de bordes · {radiusToPx(form.radius)}px</label>
            <input
              type="range"
              min={0}
              max={22}
          ...
          <div className="flex items-end gap-2">
            <div className="h-10 rounded-theme px-4 text-sm font-medium leading-10" style={{ background: form.primary, color: '#fff' }}>
              Vista previa
            </div>
            <div className="h-10 rounded-theme px-4 text-sm font-medium leading-10" style={{ background: form.secondary, color: '#1f2937' }}>
              Secundario
            </div>
          </div>
        </CardContent>
