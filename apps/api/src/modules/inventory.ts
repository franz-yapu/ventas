...
export async function inventoryRoutes(app: FastifyInstance) {
  ...
  app.patch('/inventory/:id', { preHandler: [app.requireAuth, app.requireAdmin] }, async (req, reply) => {
    ...
    if (!canAdjustInventory(user, before.locationId)) {
      return reply.code(403).send({ data: null, error: 'No puedes ajustar el inventario de esta ubicación' });
    }
    ...
    await app.audit(req, {
      ...
      after: { quantity: after!.quantity, minStock: after!.minStock, locationId: before.locationId, reason },
    });
    return reply.send({ data: after, error: null });
  });

  // POST /inventory/transfer — transferir stock entre ubicaciones (admin). Auditado.
  app.post('/inventory/transfer', { preHandler: [app.requireAuth, app.requireAdmin] }, async (req, reply) => {
    const parsed = z
      .object({
    ...
    if (!canAdjustInventory(user, fromLocationId)) {
      return reply.code(403).send({ data: null, error: 'No puedes transferir desde esa ubicación' });
    }
    ...
    try {
      ...
      if (String(e).includes('STOCK_INSUFICIENTE')) {
        return reply.code(409).send({ data: null, error: 'Stock insuficiente en la ubicación de origen' });
      }
      throw e;
    }
