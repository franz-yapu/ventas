...
- [ ] **0.1 ✓ Que el API no arranque en producción con los secretos de ejemplo.**
      ...
      *Comprobar*: test que arranca `buildApp()` con `NODE_ENV=production` sin las variables y
...
- [ ] **0.3 ✓ Subir `@fastify/jwt` a una versión que traiga `fast-jwt` ≥ 6.2.4.** Tres CVE
      ...
      *Comprobar*: `pnpm audit --prod` sin críticas + los 316 tests del API en verde.

- [ ] **0.4 ✓ Subir `react-router-dom` 6.30.4 → 6.30.5.** Redirección abierta → XSS. Es un
      parche, sin migración.

- [ ] **0.5 ✓ Subir `drizzle-orm` 0.36.4 → ≥ 0.45.2.** Inyección SQL por identificadores mal
      escapados. **Es un salto de versión de verdad**: hacerlo aparte, con la suite entera y
      `rls-integration.test.ts` como red.

- [ ] **0.6 ✓ Añadir `pnpm audit` al CI** con umbral en `high`. Habría dado las tres críticas
...
- [ ] **1.1 ✓ Validar el destino en `POST /inventory/transfer`.**
      ...
      *Comprobar*: test cross-tenant que espera 403/404 al transferir a una ubicación de otro
      negocio, y 400 con un uuid que no existe. Añadirlo a `tenant-isolation.test.ts`.

- [ ] **1.2 ✓ Limpiar las filas de inventario huérfanas que ya existan.** Hoy la base local
      está a 0 (el agente limpió), pero **hay que comprobarlo en producción** antes de dar el
      arreglo por cerrado: `select … from inventory i left join location l on l.id = i.location_id
...
- [ ] **2.1 ✓ Decidir y aplicar qué pasa al vender sin existencias.**
      ...
      *Comprobar*: test de venta concurrente del último ítem, con el comportamiento decidido.

- [ ] **2.2 ✓ Que una sucursal pueda vender.** `apps/api/src/modules/products.ts:88`: el
      ...
      *Comprobar*: test con el vendedor de Norte que ve los productos con stock en Norte.

- [ ] **2.3 ✓ Que anular una venta no borre su rastro en el arqueo.**
      ...
      que se cobró.
      *Comprobar*: test en `cash.test.ts` — vender, anular, y esperar que el descuadre aparezca.

- [ ] **2.4 ✓ Revocar las sesiones al cambiarse uno mismo la contraseña.**
      ...
      *Comprobar*: test en `sessions.test.ts` que con el refresh viejo espera 401 después.

- [ ] **2.5 ✓ Aplicar el alcance en `GET /sales?userId=`.**
      ...
      *Comprobar*: test en `alcance-sucursal.test.ts`.

- [ ] **2.6 ? Que `POST /products/import` respete la cuota del plan.**
      ...
      *Comprobar*: test en `subscription.test.ts` con un import que cruza el tope.

- [ ] **2.7 ? Que la cola offline no sobreviva al cambio de usuario.**
      ...
      *Comprobar*: test en `sync.test.ts` con cambio de usuario entre encolar y sincronizar.

---

## Bloque 3 — Alto: interfaz

- [ ] **3.1 ✓ Arreglar `--color-border` en los dos modos.** `apps/web/src/index.css:141`
...
- [ ] **3.2 ✓ Arreglar `text-muted` sobre `track`, `danger-bg` e `info-bg` en claro.**
...
- [ ] **3.3 ✓ Ampliar `paleta.test.ts` para que esto no vuelva a pasar.** Sus 27 parejas no
...
- [ ] **4.1 ✓ Arreglar `scripts/gen-api-docs.mjs`: pierde 9 de las 74 rutas reales.**
      ...
      *Comprobar*: que el generador emita tantas rutas como `printRoutes()` sin `HEAD` — y
...
- [ ] **5.13 ?** `vitest.config.ts:33` no sube `LOGIN_RATE_LIMIT_MAX` y `sessions.test.ts` roza
